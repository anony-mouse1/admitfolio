...
async function shape(seller: { bio: string | null; photoPath: string | null; backgroundTags: string }) {
  ...
  return { bio: seller.bio, backgroundTags, photoUrl };
}
...
export async function POST(req: Request) {
  ...
  let body: { bio?: string; backgroundTags?: string[] };
  try {
    body = await req.json();
  } catch {
  ...
  const seller = await prisma.seller.update({
    ...
    data: { bio, backgroundTags: JSON.stringify(backgroundTags) },
  });
